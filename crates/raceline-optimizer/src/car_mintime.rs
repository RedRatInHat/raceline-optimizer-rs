...
use crate::solver_api::{ipopt_terminal_error, SolverApiError, SolverCancelToken};
...
fn solve_car_mintime_with_ipopt_initial<'a>(
    ...
    unsafe {
        ...
        if !crate::ipopt::status_is_success(status_code) {
            return Err(ipopt_terminal_error(status_code, format!(
...
impl CarMintimeIpoptNlp<'_> {
    ...
    fn emit_preview(&mut self, x: &[f64], objective: f64, force: bool) {
        ...
        if !should_materialize_progress_preview(
            self.progress.is_some(),
            force,
            self.objective_eval_count,
            self.last_preview_eval_count,
...
mod tests {
    ...
    #[test]
    fn capability_aggregation_rejects_one_non_finite_tire() {
        let params = CarDoubleTrackParams::from_profile(&test_car_profile()).unwrap();
        let tire = crate::vehicle_dynamics::CarDoubleTrackTireForces {
            fx_fl_n: f64::NAN,
            fx_fr_n: 0.0,
            fx_rl_n: 0.0,
            fx_rr_n: 0.0,
        ...
        assert!(super::car_tire_max_active_kamm(params, tire).is_none());
    }
