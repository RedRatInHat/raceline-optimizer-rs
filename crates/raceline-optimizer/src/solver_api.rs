...
use crate::point_mass::{
    solve_point_mass_velocity_vector_ocp_classified, EnvelopeCheckPoints, PointMassProgressUpdate,
    PointMassSolveFailure, PointMassSolveOptions, PointMassSolveProgressCallback,
    PointMassSolveResult, PublishGeometryMode,
};
...
pub(crate) fn ipopt_terminal_error(status_code: i32, message: String) -> SolverApiError {
    let code = if status_code == -1 {
        "solve.nonConverged"
    } else {
        "solve.runtimeFailed"
    };
    SolverApiError::new(code, message)
}
...
pub fn solve_point_mass_json_with_progress(
    ...
    let has_progress_consumer = progress.is_some();
    emit_progress(
        &mut progress,
    ...
    let result = {
        ...
        solve_point_mass_velocity_vector_ocp_classified(
            &sections,
            &request.profile,
            &request.envelope,
            request.solve_options.clone(),
            gate_point_mass_backend_progress(has_progress_consumer, &mut solve_progress),
            cancel_token,
        )
    }
...
fn gate_point_mass_backend_progress(
    enabled: bool,
    callback: PointMassSolveProgressCallback<'_>,
) -> Option<PointMassSolveProgressCallback<'_>> {
    enabled.then_some(callback)
}
...
fn map_solver_error(error: PointMassSolveFailure) -> SolverApiError {
    let status_code = error.ipopt_status_code();
    let message = error.into_message();
    if message == "solve.cancelled" {
        return SolverApiError::new("solve.cancelled", "point mass optimizer cancelled");
    }
    if message.contains("Ipopt dynamic backend")
        || message.contains("failed to load Ipopt")
        || message.contains("missing Ipopt symbol")
    {
        return SolverApiError::new("solve.nativeBackendUnavailable", message);
    }
    if let Some(status_code) = status_code {
        return ipopt_terminal_error(status_code, message);
    }

    SolverApiError::new("solve.runtimeFailed", message)
}
...
mod tests {
    ...
    #[test]
    fn ipopt_iteration_exhaustion_is_not_a_missing_backend() {
        let message = "Maximum_Iterations_Exceeded (-1)".to_owned();
        let vehicle_error = ipopt_terminal_error(-1, message.clone());
        assert_eq!(vehicle_error.code, "solve.nonConverged");
        assert_eq!(vehicle_error.message, message);
        assert_eq!(
            map_solver_error(PointMassSolveFailure::ipopt_terminal(
                -1,
                "terminal wording does not determine classification".to_owned()
            ))
            .code,
            "solve.nonConverged"
        );
    ...
    #[test]
    fn ipopt_runtime_errors_are_distinct_from_loading_and_cancellation() {
        for status in [-2, -3, -10, -12, -13, -100, -199] {
            assert_eq!(
                ipopt_terminal_error(status, "optimizer failure".to_owned()).code,
                "solve.runtimeFailed"
            );
        }
        assert_eq!(
            map_solver_error("failed to load Ipopt".to_owned().into()).code,
            "solve.nativeBackendUnavailable"
        );
        assert_eq!(
            map_solver_error("solve.cancelled".to_owned().into()).code,
            "solve.cancelled"
        );
        assert_eq!(
            map_solver_error(PointMassSolveFailure::ipopt_terminal(
    ...
    #[test]
    fn point_failure_text_cannot_impersonate_an_ipopt_status() {
        let error = map_solver_error(
            "invalid request value: Maximum_Iterations_Exceeded (-1)"
                .to_owned()
                .into(),
        );
    ...
    #[test]
    fn point_mass_backend_callback_is_absent_without_external_consumer() {
        let mut disabled = |_update: PointMassProgressUpdate| panic!("no consumer");
        assert!(gate_point_mass_backend_progress(false, &mut disabled).is_none());
        let mut enabled = |_update: PointMassProgressUpdate| {};
        assert!(gate_point_mass_backend_progress(true, &mut enabled).is_some());
    }
