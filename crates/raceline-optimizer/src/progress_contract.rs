...
pub(crate) fn should_materialize_progress_preview(
    has_callback: bool,
    force: bool,
    objective_eval_count: u32,
    last_preview_eval_count: u32,
    preview_eval_period: u32,
) -> bool {
    has_callback
        && (force
            || objective_eval_count.saturating_sub(last_preview_eval_count) >= preview_eval_period)
}
...
impl SolverCapabilityEventV2 {
    pub fn from_max_utilization(
        ...
        if !max_utilization.is_finite()
            || !clean_limit.is_finite()
            || clean_limit <= 0.0
            || objective.is_some_and(|value| !value.is_finite())
        {
            return None;
        }
...
mod tests {
    use super::{
        finite_max, should_materialize_progress_preview, SolverCapabilityEventV2,
        SolverConvergenceEventV2,
    };
    use crate::json::JsonValue;
    ...
    #[test]
    fn finite_max_rejects_empty_or_partially_non_finite_samples() {
        assert_eq!(finite_max([0.83, 1.04]), Some(1.04));
        assert_eq!(finite_max(std::iter::empty()), None);
        assert_eq!(finite_max([0.83, f64::NAN, 1.04]), None);
    }

    #[test]
    fn preview_materialization_requires_a_callback_and_respects_throttling() {
        assert!(!should_materialize_progress_preview(false, true, 1, 0, 25));
        assert!(!should_materialize_progress_preview(
            false, false, 25, 0, 25
        ));
        assert!(should_materialize_progress_preview(true, true, 1, 0, 25));
        assert!(!should_materialize_progress_preview(true, false, 24, 0, 25));
        assert!(should_materialize_progress_preview(true, false, 25, 0, 25));
    }
